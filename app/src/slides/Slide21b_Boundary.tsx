...
const BOUNDARIES = [
  ...
  },
  {
    ...
    color: '#8B5CF6',
    desc: '模型智能有上限，框架也有局限',
    action: '氪金用更好的模型，Coding Plan 省很多，等待技术发展',
  },
  {
    title: '框架的能力边界',
    icon: '🛠',
...
const Slide21b_Boundary: React.FC<SlideProps> = ({ isActive }) => {
  ...
  useGSAP(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.bd-title', { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 0.6 });
      tl.fromTo('.bd-subtitle', { opacity: 0, y: 10 }, { opacity: 1, y: 0, duration: 0.5 }, 0.3);
  ...
  return (
    <section ref={containerRef}
      ...
      <h2 className="bd-title text-h1 md:text-display font-bold text-[var(--text-primary)] mb-2 opacity-0">
        在使用中体验边界
      </h2>

      <p className="bd-subtitle text-body text-[var(--text-secondary)] text-center max-w-3xl mb-6 opacity-0">
        这个边界很模糊——可能是你能力的边界限制了效果，也可能是模型或框架的边界
      </p>
      ...
      <div className="bd-bottom mt-auto pt-5 max-w-3xl w-full text-center opacity-0">
        <div className="rounded-xl border-2 px-6 py-4"
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            去尝试、去学习——只有实际使用和投入精力调教，才有真正的手感与经验
          </p>
        </div>
      </div>
    </section>
...
export default memo(Slide21b_Boundary);
